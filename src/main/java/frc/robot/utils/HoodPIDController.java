...
public class HoodPIDController implements Sendable, AutoCloseable {
   ...
   */
  public HoodPIDController(double kp, double ki, double kd) {
    this(kp, ki, kd, 0.02);
  }
   ...
   */
  public HoodPIDController(double kp, double ki, double kd, double period) {
    m_kp = kp;
    m_ki = ki;
    m_kd = kd;
    ...
    if (m_ki != 0) {
      ...
      m_totalError =
          MathUtil.clamp(
              ...
              if (m_totalError * (m_setpoint - m_measurement) < 0)
              {
                  m_totalError = 0;
              }
    }
